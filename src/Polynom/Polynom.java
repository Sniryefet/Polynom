...
public class Polynom extends JFrame implements Polynom_able {
	 ...
	 * 
	 * @author Barel Katsepolsky && Snir Yefet
	 * 
	 * 
	 */
	...
	/**
	 * Constructor which gets Polynom and duplicate it
	 * @param p The Polynom which we are building copy of 
	 */
	public void Plot (double Start , double End) {
        ...
        DataTable CritData =new DataTable(Double.class,Double.class);
        Polynom dev =new Polynom((Polynom) this.derivative());
        Polynom secDev =new Polynom((Polynom)dev.derivative());
        
        for(double x =Start ;x<=End; x+=0.1) {
        	double y =f(x);
        	if((f(x)<=f(x-0.1)&&f(x)<=f(x+0.1))||(f(x)>=f(x-0.1)&&f(x)>=f(x+0.1))) {    		
                CritData.add(x,y);
                System.out.printf("(%.5f ,%.5f)", x ,y);
        	}    	
        }
        XYPlot plot = new XYPlot(data,CritData);
        ...
        plot.getPointRenderers(CritData).get(0).setColor(color2);
	 ...
	 * @throws Exception In case we the String we've got is Invalid
	 */
		...
		while(itr.hasNext()) {
			...
			if(m.get_power()==m1.get_power()) {
				...
				if(m.get_coefficient()==0)
					itr.remove();                    //if m.coefficient was -m1.coefficient we want to remove
	...
	public boolean equals(Polynom_able p1) {
		...
		if((itr1.hasNext()&&!itr2.hasNext())||(!itr1.hasNext()&&itr2.hasNext())) //there is still a chance which we finished checking and the
